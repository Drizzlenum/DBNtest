...
import numpy as np
from util2 import tf_xavier_init, sample_bernoulli, sample_gaussian, batch_generator
...
class BBRBM:
    ...
    def sample_v_given_h(self, h_0_p):
        return tf.nn.sigmoid(tf.matmul(sample_bernoulli(h_0_p), tf.transpose(a=self.w)) + self.vbias)

    def compute_visible_func(self, hidden):
        return tf.nn.sigmoid(tf.matmul(hidden, tf.transpose(a=self.w)) + self.vbias)

    def free_energy_func(self):
        return -tf.reduce_mean(input_tensor=tf.matmul(self.x, tf.expand_dims(self.vbias, 1))) - tf.reduce_mean(input_tensor=tf.math.log(1 + tf.exp(tf.matmul(self.x, self.w) + self.hbias)))
    ...
    def _build_model(self):
        ...
        positive_grad = tf.matmul(tf.transpose(a=self.x), h_0_p)
        negative_grad = tf.matmul(tf.transpose(a=v_t), h_t_p)

        accum_delta_w = (positive_grad - negative_grad) / tf.cast(tf.shape(input=self.x)[0], dtype=tf.float32)
        accum_delta_vbias = tf.reduce_mean(input_tensor=self.x - v_t, axis=0)
        accum_delta_hbias = tf.reduce_mean(input_tensor=h_0_p - h_t_p, axis=0)
...
class GBRBM(BBRBM):
    ...
    def sample_v_given_h(self, h_0_p):
        return tf.matmul(sample_bernoulli(h_0_p), tf.transpose(a=self.w)) + self.vbias

    def compute_visible_func(self, hidden):
        return tf.matmul(hidden, tf.transpose(a=self.w)) + self.vbias

    def free_energy_func(self):
        return - tf.reduce_mean(input_tensor=0.5 * tf.matmul((self.x - self.vbias),
        tf.transpose(a=self.x - self.vbias))) \
        - tf.reduce_mean(input_tensor=tf.math.log(1 + tf.exp(tf.matmul(self.x, self.w) + self.hbias)))
